...
test("the owner journey renders only the closed document and keeps raw source in its textarea", async () => {
  ...
  assert.match(form, /<PageImportTextField\s+disabled=\{busy\}\s+error=\{fieldErrors\.title\}\s+label=\{copy\.pageTitle\}\s+maxLength=\{200\}\s+name="title"\s+onInput=\{clearFieldError\}\s+required\s+optionalSuffix=\{copy\.optional\}\s+\/>/u);
  assert.match(form, /<PageImportTextareaField\s+disabled=\{busy\}\s+error=\{fieldErrors\.description\}\s+label=\{copy\.description\}\s+maxLength=\{500\}\s+name="description"\s+onInput=\{clearFieldError\}\s+optional\s+required=\{false\}\s+rows=\{3\}\s+optionalSuffix=\{copy\.optional\}\s+\/>/u);
  assert.match(form, /<PageImportTextareaField\s+disabled=\{busy\}\s+error=\{fieldErrors\.htmlFragment\}\s+errorId="page-preview-fragment-error"\s+help=\{copy\.htmlErrorMessage\}\s+helpId="page-preview-fragment-help"\s+label=\{copy\.htmlFragment\}\s+name="htmlFragment"\s+onInput=\{clearFieldError\}\s+required\s+rows=\{16\}\s+spellCheck=\{false\}\s+variant="source"\s+optionalSuffix=\{copy\.optional\}\s+\/>/u);
  ...
  assert.match(form, /<textarea\s+aria-label=\{copy\.normalizedLabel\}[\s\S]*value=\{JSON\.stringify\(document, null, 2\)\}/u);
  ...
  assert.match(renderer, /copy\.linkOpensNewTab/u);
  assert.match(shell, /href="\/owner\/pages\/import"/u);
  assert.match(shell, /copy\.pages/u);
