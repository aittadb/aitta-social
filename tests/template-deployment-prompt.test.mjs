...
test("a truly unconfigured deployment leads with the exact prompt and published-only updates", async () => {
  ...
  assert.match(html, /aria-label="Set up this Aitta — sign in with ChatGPT for local sole-owner administration"[^>]*>Set up<\/a>/i);
...
test("the prompt reveals no owner authorization result", async (t) => {
  ...
  await t.test("signed-in owner gets the normal owner destination", async () => {
    ...
    assert.match(html, /href="\/owner"[^>]+aria-label="Set up this Aitta — open local sole-owner administration"[^>]*>Set up<\/a>/i);
  ...
  await t.test("another signed-in visitor gets no administrative claim or private view", async () => {
    ...
    assert.match(publicHtml, /href="\/owner"[^>]+aria-label="Set up this Aitta — open local sole-owner administration"[^>]*>Set up<\/a>/i);
...
test("a D1 failure is unavailable Aitta storage, never fresh setup or an owner claim", async (t) => {
  ...
  const cases = [
    ["signed out", {}, /href="\/signin-with-chatgpt\?return_to=%2Fowner"[^>]*>Manage Aitta<\/a>/i],
    ["signed-in owner", ownerHeaders(ownerEmail), /href="\/owner"[^>]*>Manage Aitta<\/a>/i],
    ["signed-in non-owner", ownerHeaders(otherEmail), /href="\/owner"[^>]*>Manage Aitta<\/a>/i],
  ];
