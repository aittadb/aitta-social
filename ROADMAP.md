...
The current `PLAN.md` ends with a production-equivalent presence-first release
...
- [ ] **ROADMAP-004 — Establish secure optional Hub registration and disconnect.** Let the sole local owner register and later disconnect one independently deployed presence without pasted secrets, shared content storage, or loss of local public operation. This preserves unfinished provenance from former `TASK-042`, `TASK-043`, `TASK-062`, `TASK-063`, `TASK-066`, `TASK-067`, `TASK-086`, `TASK-095`, `TASK-096`, and `TASK-097`.
- [ ] **ROADMAP-005 — Add verified presence discovery.** Make the presence's public protocol consumable by Hub, then let the owner resolve and browse minimal opted-in verified presence metadata without rankings, identity-truth claims, or a local directory copy. This preserves unfinished provenance from former `TASK-041`, `TASK-045`, `TASK-046`, `TASK-071`, `TASK-072`, `TASK-087`, and `TASK-088`.
- [ ] **ROADMAP-006 — Add explicit one-way Follow and Unfollow.** Use a confined acting-presence session so the owner can deliberately create or remove one Hub-owned edge, with no automatic or reciprocal relationship, popularity count, or public graph. This preserves unfinished provenance from former `TASK-047`, `TASK-048`, `TASK-073`, `TASK-074`, `TASK-075`, and `TASK-131`.
...
`ROADMAP-001` through `ROADMAP-003`. Completed presence-first work from that
