...
- [ ] **TASK-225 — Centralize owner-form ARIA description composition.** This is a behavior-preserving owner-form maintainability slice. DoD: add one dependency-free `app/owner/form-field-description.ts` leaf exporting canonical `describedBy(...ids: Array<string | undefined>): string | undefined`; preserve the exact truthy filtering, input order, repeated identifiers, whitespace-containing identifiers, single-space joining, and `undefined` result when no truthy identifier exists; replace the two byte-identical declarations in `app/owner/entries/EntryForm.tsx` and `app/owner/profile/ProfileForm.tsx` with imports while retaining all eight existing call sites, help and error identifiers, labels, validation state, and rendered `aria-describedby` behavior; add focused empty, undefined, empty-string, whitespace, ordered, and repeated-identifier behavior plus canonical-declaration, two-consumer-import, local-declaration-removal, cumulative lint-matrix, and cross-family regression evidence; reject top-level `describedBy` declarations outside the canonical leaf across supported function, class, and variable declaration forms using `restrictedSyntaxErrorCount`, keep every prior restriction active inside the canonical file, and prove TASK-224 `consumeResponse` ownership remains active in both canonical directions; add no dependency or observable production behavior, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the focused helper and owner-form consumer evidence, repository checks, and full validation. Depends on: TASK-224.
