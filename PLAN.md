...
- [ ] **TASK-211 — Centralize custom-page text boundary primitives.** This is a behavior-preserving production-maintainability slice. DoD: add dependency-free, narrowly named `lib/custom-pages/page-text-boundaries.ts` exporting canonical `characterLength`, `hasForbiddenTextControl`, and `hasUrlControl`; preserve exact Unicode code-point counting, text-control behavior that permits only tab, LF, and CR among C0 controls, and URL-control behavior that rejects C0 controls, space, and DEL; replace both declaration sets in the HTML-fragment compiler and PageDocument validator with imports; canonicalize the byte-identical `visibleInlineText` / `inlineVisibleText` recursion as `pageInlineVisibleText` in `lib/custom-pages/page-document.ts` and import it into the compiler without adding a circular module dependency; preserve every existing limit, HTML-import rejection, validator-false result, URL normalization, fragment rule, error, route, and public/private contract; add focused Unicode/control/visible-text behavior, compiler and document-validator regression, canonical-declaration, consumer-import, and lint evidence; reject top-level canonical and legacy helper-name redeclarations outside their canonical owners across function, class, and variable declaration forms without weakening any earlier duplicate-helper restriction; add no dependency, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass focused custom-page tests and full validation. Depends on: TASK-210.
