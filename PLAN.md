...
- [ ] **TASK-209 — Centralize test error-document expectations.** This is test-maintainability only. DoD: add dependency-free, narrowly named `tests/helpers/error-document-contract.mjs` exporting canonical `errorDocument`; preserve the exact fresh `{ data: null, error: { code, message }, links: [] }` envelope with code and message unchanged; replace the six byte-identical declarations in API v1 root, API v1 entry detail, API v1 collection, API v1 profile, public entry document, and private entry deletion tests with imports; add focused envelope/freshness, canonical-declaration, consumer-import, and lint regression evidence; reject top-level `errorDocument` redeclarations in test `.mjs` files outside the canonical helper across function, class, and variable declaration forms without weakening record-shape, regular-expression, public-footer, Accept-parser, or private-JSON restrictions; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the six focused consumers plus helper test and full validation. Depends on: TASK-208.
