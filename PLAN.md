...
- [ ] **TASK-207 — Centralize public-footer contract extraction in tests.** This is test-maintainability only. DoD: add one dependency-free, narrowly named `tests/helpers/public-footer-contract.mjs` leaf exporting canonical `publicFooter`; preserve the current exact behavior of returning the first case-insensitive `<footer class="public-footer">…</footer>` match or `""` when absent; replace the four identical declarations in `tests/technical-page.test.mjs`, `tests/privacy-page.test.mjs`, `tests/accessibility-contract.test.mjs`, and `tests/public-frame.test.mjs` with imports; add focused extraction behavior, canonical-declaration, consumer-import, and lint regression evidence; reject top-level `publicFooter` redeclarations in test `.mjs` files outside the canonical helper without weakening existing record-shape, regular-expression, or production-component rules; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the four focused consumers and full validation. Depends on: TASK-206.
