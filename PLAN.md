...
- [ ] **TASK-153 — Keep empty-state headings subordinate at narrow widths.** Correct the rendered hierarchy defect found by TASK-137 without changing empty-state meaning or the accepted primary headline scale. DoD: keep the public empty-update h3 smaller than its Updates h2 and the owner empty-update h3 smaller than its Updates h2 at wide, actual 320-pixel, and 400-percent-reflow-equivalent layouts; preserve semantic heading levels, long-content wrapping, touch targets, contrast, reduced motion, and all populated/card typography; add relational source and rendered regressions plus focused documentation; record no schema, migration, protocol, runtime-configuration, hosting, or external-state change; and pass focused and full validation. Depends on: TASK-151.
