...
- [ ] **TASK-215 — Centralize ESLint restricted-syntax diagnostic counting in tests.** This is test-maintainability only. DoD: add dependency-free `tests/helpers/eslint-restricted-syntax.mjs` exporting `restrictedSyntaxErrorCount(...results)`, counting exactly messages whose `ruleId` is `no-restricted-syntax`; replace the eight local `restrictedSyntaxErrors` declarations and five equivalent inline counters with imports/calls while retaining every local lint fixture, canonical allowance, cross-family restriction, and expected count; add focused zero/mixed/batched diagnostic behavior, canonical declaration, consumer import, legacy-name removal, and lint regression evidence; reject top-level `restrictedSyntaxErrorCount` and legacy `restrictedSyntaxErrors` declarations outside the canonical helper across supported function, class, and variable declaration forms without weakening prior restrictions; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass focused consumers and full validation. Depends on: TASK-214.
