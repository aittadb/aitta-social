...
- [ ] **TASK-208 — Centralize private JSON response contract assertions in tests.** This is test-maintainability only. DoD: add dependency-free, narrowly named `tests/helpers/private-json-response.mjs` exporting canonical `assertPrivateJson`; preserve status equality, the `application/json` content-type boundary, `cache-control: no-store`, and `Vary`-token `Accept` checks; retain the six current case-insensitive content-type checks by default and the custom-page preview’s current case-sensitive check through an explicit narrow option; replace declarations in exactly the private-profile, private-entry create/edit/state/delete, custom-page preview, and delete-lifecycle consumer tests with imports; add focused default/strict behavior, canonical-declaration, consumer-import, and lint regression evidence; reject top-level `assertPrivateJson` redeclarations in test `.mjs` files outside the canonical helper across function and class forms without weakening record-shape, regular-expression, Accept-parser, or public-footer restrictions; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the seven focused consumers plus helper test and full validation. Depends on: TASK-207.
