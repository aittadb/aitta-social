...
- [ ] **TASK-218 — Centralize regular-expression match counting in tests.** This is test-maintainability only. DoD: add one dependency-free, narrowly named `tests/helpers/regular-expression-match-count.mjs` leaf exporting canonical `countMatches`; preserve the exact `[...value.matchAll(pattern)].length` behavior, including zero, single, multiple, case-insensitive, zero-width, and non-global-RegExp rejection semantics; replace the two byte-identical declarations in `tests/assisted-runtime-journey.test.mjs` and `tests/update-composer.test.mjs` plus the behaviorally equivalent `value.match` declaration in `tests/first-update-journey.test.mjs` after proving every current caller supplies a global RegExp literal, while retaining every consumer-owned pattern and expected count; add focused helper behavior, canonical-declaration, three-consumer-import, legacy-declaration-removal, and lint regression evidence; reject top-level `countMatches` declarations outside the canonical helper across the supported function, class, and variable declaration forms without weakening prior restrictions; use `restrictedSyntaxErrorCount` for lint evidence; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the helper test, all three consumer suites, repository checks, and full validation. Depends on: TASK-217.
