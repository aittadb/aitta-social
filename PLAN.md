...
- [ ] **TASK-217 — Centralize CSS clamp pixel calculations in tests.** This is test-maintainability only. DoD: add one dependency-free, narrowly named `tests/helpers/css-clamp-pixels.mjs` leaf exporting canonical `clampPixels`; preserve the exact `Math.min(maxRem * 16, Math.max(minRem * 16, width * preferredVw / 100))` behavior and its existing 16-pixel rem assumption; replace the two byte-identical declarations in `tests/empty-heading-hierarchy.test.mjs` and `tests/headline-scale.test.mjs` with imports; add focused minimum-clamped, fluid, maximum-clamped, and exact-boundary behavior plus canonical-declaration, consumer-import, legacy-declaration-removal, and lint regression evidence; reject top-level `clampPixels` declarations outside the canonical helper across the supported function, class, and variable declaration forms while proving the canonical exception does not weaken an existing cross-family restriction; use `restrictedSyntaxErrorCount` for lint evidence; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the helper test, both consumer suites, repository checks, and full validation. Depends on: TASK-216.
