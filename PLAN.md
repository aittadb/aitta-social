...
- [ ] **TASK-205 — Centralize test regular-expression literal escaping.** This is test-maintainability only. DoD: add one dependency-free, narrowly named `tests/helpers/regular-expression-literal.mjs` leaf exporting canonical `escapeRegExp`; replace all four `escapeRegExp` and four `escapeRegex` declarations with imports; preserve the exact escaped output used by every existing `RegExp` construction, including regex metacharacters, ordinary text, slashes, and Unicode input; add focused behavior, canonical-declaration, consumer-import, and lint regression evidence; reject top-level `escapeRegExp` and legacy `escapeRegex` redeclarations outside the canonical helper without weakening the existing record-shape lint restrictions; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass focused consumers, repository checks, and full validation. Depends on: TASK-204.
