...
- [ ] **TASK-220 — Centralize inline-style attribute extraction in tests.** This is test-maintainability only. DoD: add one dependency-free `tests/helpers/inline-style-attribute-values.mjs` leaf exporting canonical `inlineStyleAttributeValues(html)` with the exact existing whitespace-prefixed, case-insensitive, double-quoted `style` attribute extraction and source-order behavior; replace the two byte-identical `styleAttributes` declarations in `tests/identity-appearance.test.mjs` and `tests/presentation-accent.test.mjs` with imports while retaining each consumer’s HTML fixture, accent resolution, persistence, private-canary pattern, and assertion ownership; add focused absent, ordered, repeated, uppercase, empty-value, `data-style`, and single-quoted behavior plus canonical-declaration, two-consumer-import, legacy-removal, cumulative lint-matrix, and cross-family regression evidence; reject top-level `inlineStyleAttributeValues` outside the canonical helper and legacy `styleAttributes` globally across supported function, class, and variable declaration forms using `restrictedSyntaxErrorCount`, without weakening prior restrictions; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the helper and both consumer suites, repository checks, and full validation. Depends on: TASK-219.
