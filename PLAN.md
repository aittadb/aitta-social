...
- [ ] **TASK-152 — Establish `develop` as the feature-integration base.** Repository workflow change requested before the remaining feature work merges: create local and remote `develop` from the exact current `origin/main`, keep direct main updates prohibited, and make complete feature branches rebase onto `develop` before review. DoD: prove `develop` and `origin/develop` initially equal the fetched `origin/main`; document feature branches targeting and rebasing onto `origin/develop`, owner-reviewed release promotion from `develop` to `main`, and fresh post-merge branch rules; inspect open pull requests plus local and remote feature refs, rebase only clean validated branches that are actually merge-ready, and leave unfinished, superseded, or already-integrated task branches untouched; record the no-op ancestry result when a candidate already contains `develop`; make no runtime, schema, protocol, hosting, deployment, data, setting, access, DNS, or domain change; and pass repository validation. Depends on: none.
