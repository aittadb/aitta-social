...
- [ ] **TASK-203 — Co-locate owner-shell CSS and establish stylesheet ownership.** This is a source-only maintainability refactor. DoD: add concise repository guidance that component-specific styling belongs in an adjacent CSS module while global or theme-specific files retain color and typography tokens, resets, accessibility foundations, and genuinely cross-component primitives; move only `OwnerShell` and `OwnerAccessState` header, navigation, frame, footer, access-state, nested-footer, and responsive rules from `app/globals.css` into `app/owner/_components/OwnerShell.module.css`; consume them through explicit module classes, use existing global variables or system colors without defining component-local colors, and remove the migrated global selectors without duplication; preserve the exact authorized, denied, and unconfigured landmarks, copy, links, active navigation, 44-pixel targets, safe-area layout, focus, reduced-motion, forced-colors, narrow-layout, and rendered appearance; update focused source and rendered ownership regressions to inspect the adjacent module and prove the migrated selectors are absent from `globals.css`; add no dependency, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass focused tests, agent/plan checks, typecheck, lint, build, full validation, and diff checks. Depends on: none.
