...
- [ ] **TASK-216 — Centralize ordered-text test assertions.** This is test-maintainability only. DoD: add one dependency-free, narrowly named `tests/helpers/ordered-text-assertion.mjs` leaf exporting canonical `assertOrdered`; preserve the existing sequential `indexOf` behavior, including repeated markers and the current assertion message; replace the two byte-identical declarations in `tests/public-hierarchy.test.mjs` and `tests/empty-heading-hierarchy.test.mjs` with imports; add focused ordered, missing, reversed, repeated, and empty-needle behavior plus canonical-declaration, consumer-import, and lint regression evidence; reject top-level `assertOrdered` declarations in test `.mjs` files outside the canonical helper across function, class, and variable declaration forms without weakening prior restrictions; use `restrictedSyntaxErrorCount` for lint regression evidence; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass both consumers, helper test, repository checks, and full validation. Depends on: TASK-215.
