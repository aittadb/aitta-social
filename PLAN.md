...
- [ ] **TASK-202 — Extract Technical page information sections into a focused component.** This is a source-only maintainability refactor. DoD: add one narrowly typed, feature-local `TechnicalInformationSection` child component that owns the existing semantic section class, `aria-labelledby`, and matching `h2` identifier relationship; render all four Technical page sections through it while `TechnicalPage` retains their exact content and order; preserve the exact metadata, frame, article, header, heading identifiers and copy, paragraphs, native-link destinations, labels and classes, D1 independence, cache/security/privacy behavior, and rendered accessibility; add focused component-composition and rendered-contract regression evidence; add no shared UI framework, dependency, route, API, CSS, schema, migration, persistence, hosting, deployment, access, data, or external-state change; pass focused tests, typecheck, lint, build, repository checks, and full validation. Depends on: none.
