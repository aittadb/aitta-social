...
- [ ] **TASK-213 — Centralize deletion acknowledgement fixtures.** This is test-maintainability only. DoD: add one dependency-free, narrowly named `tests/helpers/deletion-acknowledgement-contract.mjs` leaf exporting canonical `deletionAcknowledgement`; preserve the exact fresh deletion acknowledgement envelope, including its owner-entry-deletion type, deleted attribute, collection and recovery HTML links, and empty actions; replace all three local `deletionAcknowledgement` and one legacy `acknowledgement` declarations with imports while retaining test-specific assertion wrappers; add focused envelope/freshness, canonical-declaration, consumer-import, and lint regression evidence; reject top-level canonical and legacy fixture-name redeclarations in test `.mjs` files outside the canonical helper across function, class, and variable declaration forms without weakening earlier restrictions; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the four deletion-focused consumers and full validation. Depends on: TASK-212.
