...
- [ ] **TASK-214 — Centralize API v1 HEAD response-header assertions.** This is test-maintainability only. DoD: add dependency-free, narrowly named `tests/helpers/api-v1-head-response.mjs` exporting `assertMatchingApiV1HeadHeaders(head, get)`, which compares exactly `content-type`, `cache-control`, `vary`, `allow`, and `location` by header value; replace collection/detail/profile `assertMatchingHeaders` and root `assertMatchingHeadHeaders` with imports; rename the semantically broader public-entry-document local assertion to `assertMatchingEntryDocumentHeaders` while retaining its CSP comparison and all callers; add focused exact-header pass/fail, canonical-declaration, consumer-import/legacy-name-removal, and lint regression evidence; reject top-level canonical and legacy names `assertMatchingHeaders`/`assertMatchingHeadHeaders` in test `.mjs` files outside the canonical helper across function, class, and variable declaration forms without weakening earlier restrictions; add no dependency or production, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass four API v1 consumers plus public-entry-document and full validation. Depends on: TASK-213.
