...
- [ ] **TASK-224 — Centralize discarded response-body consumption in tests.** This is test-maintainability only. DoD: add one dependency-free `tests/helpers/response-body-consumption.mjs` leaf exporting canonical async `consumeResponse(response)` that awaits exactly one `response.text()` call, discards the resolved body, returns `undefined`, and propagates the original rejection; replace the two byte-identical declarations in `tests/presence-functional-matrix.test.mjs` and `tests/upgrade-preservation.test.mjs` with imports while retaining all seven existing call sites and their surrounding status, authorization, persistence, privacy, and lifecycle assertions; add focused exactly-once, awaited-completion, discarded-result, rejection-propagation, canonical-declaration, two-consumer-import, local-declaration-removal, cumulative lint-matrix, and cross-family regression evidence; reject top-level `consumeResponse` declarations outside the canonical helper across supported function, class, and variable declaration forms using `restrictedSyntaxErrorCount`, keep every prior restriction active inside the canonical file, and do not weaken TASK-223 Vary-token restrictions; import no production implementation or test harness and add no dependency or production behavior, route, API, schema, migration, persistence, hosting, deployment, data, access, or external-state change; pass the helper and two consumer suites, repository checks, and full validation. Depends on: TASK-223.
