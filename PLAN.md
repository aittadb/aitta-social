...
- [ ] **TASK-204 — Centralize exact record-shape boundary predicates.** This is a source-only maintainability refactor. DoD: add one dependency-free `lib/record-shape.ts` leaf that solely declares and exports the existing `isRecord` and `hasExactKeys` predicates; replace all eight current local `isRecord` declarations and all seven current `hasExactKeys` declarations with imports while preserving their exact type narrowing, own-enumerable-key comparison, strict response/document validation, rejection behavior, and browser/Worker compatibility; adapt only the direct TypeScript data-URL test harnesses through a narrowly named ESM test compiler helper so the new runtime import resolves without inlining or redeclaring either predicate; add focused predicate/import-ownership regressions, concise AGENTS guidance against redeclaring exact generic helpers, and configure existing ESLint (no dependency) to reject top-level function/arrow declarations named `isRecord` or `hasExactKeys` outside the canonical module; add no generic catch-all utilities, route, API, schema, migration, persistence, hosting, deployment, or external-state change; pass focused tests, agents/plan/runtime checks, typecheck, lint, build, and full validation. Depends on: none.
